  ...
  return (
    <div className="-mt-8 pb-16">
      ...
      <div className="border-y border-border bg-surface">
        <Section className="py-16 sm:py-20">
          <h2 className="text-balance font-display text-3xl font-extrabold text-navy sm:text-4xl">
            Built with red lines.
          </h2>
          <p className="mt-3 max-w-2xl text-pretty text-muted">
            A trust tool can quietly become a surveillance tool. Civitas is built so it can&apos;t.
          </p>
          <div className="mt-8 grid gap-x-8 gap-y-3 sm:grid-cols-2">
            {[
              'No “bad member” score — signals describe the community, not people.',
              'Automation flags; humans decide. No black-box verdicts.',
              'Every decision gets an explanation and an appeal.',
              'No private user data in public dashboards.',
              'Report volume alone never triggers action.',
              'Screening is transparent pattern-matching — a prompt, not truth.',
            ].map((line) => (
              <div key={line} className="flex items-start gap-2.5">
                <Check className="mt-0.5 h-4 w-4 shrink-0 text-teal" aria-hidden />
                <span className="text-pretty text-sm leading-relaxed text-ink/90">{line}</span>
              </div>
            ))}
          </div>
        </Section>
