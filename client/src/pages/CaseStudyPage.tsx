...
export function CaseStudyPage() {
  return (
    <article className="mx-auto max-w-2xl py-4">
      ...
      <H2>Where this goes: Trust Receipts for the information economy</H2>
      <P>
        The current demo is intentionally focused: it shows how online communities can make moderation and governance
        decisions transparent through Decision Receipts. That focus matters — a working community demo is more credible
        than a broad vision with no artifact behind it.
      </P>
      <P>
        But the underlying idea is larger. A Decision Receipt is a specific form of a wider object: a <strong>Trust
        Receipt</strong> that attaches reasoning to any consequential claim, rating, or decision — what was decided, the
        standard or evidence used, what remains uncertain, who reviewed it, and how to challenge it.
      </P>
      <P>In future versions, the same model could be adapted for:</P>
      <ul className="mt-3 list-disc space-y-2 pl-5">
        <LI>media-literacy workshops that teach students to audit claims, not just consume verdicts;</LI>
        <LI>financial and economic explainers that separate measurable facts from interpretation;</LI>
        <LI>ESG statements that distinguish verified evidence from broad positioning;</LI>
        <LI>public institutions that need clearer communication and appeal paths;</LI>
        <LI>AI-assisted systems where automated decisions require human-readable reasoning.</LI>
      </ul>
      <P>
        Those are roadmap directions, not current product claims. The demo proves the first piece — trust improves when
        people can see the reasoning — and the broader ambition is to make that standard portable across the information
        economy. <strong>Trust should not be a black box.</strong>
      </P>
